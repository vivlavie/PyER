...
class R3d(object):
    ...
    # x,y,z will always return cell centres (if there are cells)
    ...
    def _center_coords(self, axis):
        """Return write-protected view of centre coordinates on the given axis."""
    ...
    def _staggered_coords(self, axis):
        ...
        if self.r3dtype == 1:
            ...
            retview = cellcentres_to_gridlines(rawcoord_center)[1:]
        elif self.r3dtype == 2:
    ...
    def new_grid(self, x, y, z, **kwargs):
        ...
        # Find cell centres and gridlines
        if coords == 'centres':
            centre_x = x
            centre_y = y
            centre_z = z
            gridlines_x = cellcentres_to_gridlines(x)
            gridlines_y = cellcentres_to_gridlines(y)
            gridlines_z = cellcentres_to_gridlines(z)
        elif coords == 'gridlines':
            centre_x = gridlines_to_cellcentres(x)
            centre_y = gridlines_to_cellcentres(y)
            centre_z = gridlines_to_cellcentres(z)
            gridlines_x = x
            gridlines_y = y
            gridlines_z = z
            ...
            errstr = ('coords argument must be \'centres\', \'gridlines\'' + 
                     ' or \'baregridlines\', not \'{0}\'')
            raise Error(errstr.format(coords))

        # Create the new R3d instance
        newr3d = R3d(centre_x, centre_y, centre_z, [], [], t=self.t)
    ...
    def radar(self,fieldID,isovalue, centre, 
        ...
        rmax_list = radar(data,grid,isovalue,centre,zrange,n,exposed,elevation)
        return rmax_list

    def compute_gridlines(self,axis=None):
        """Return gridlines used to generate this r3d file"""
        if axis == 'x':
            gridlines = cellcentres_to_gridlines(self.x)
        elif axis == 'y':
            gridlines = cellcentres_to_gridlines(self.y)
        elif axis == 'z':
            gridlines = cellcentres_to_gridlines(self.z)
        elif axis == None:
            gridlines = [cellcentres_to_gridlines(self.x),
                         cellcentres_to_gridlines(self.y),
                         cellcentres_to_gridlines(self.z)]
        else:
...
def radar(data, grid, isovalue, centre, 
            zrange=(-inf,inf), n=24, exposed='above', elevation=None):
    ...
    dx_list = grid[0] - centre[0]
    dy_list = grid[1] - centre[1]
...
def gridlines_to_cellcentres(gridline_list):
    """Return a list of cell centres given a sequence of gridlines.
...
def cellcentres_to_gridlines(centre_list):
    """Return a list of gridlines given a sequence of cell centres.
    ...
    """
    gridline_list_left = np.zeros(len(centre_list) + 1)
    gridline_list_right = np.zeros(len(centre_list) + 1)
    gridline_list_left[0] = centre_list[0]
    gridline_list_right[-1] = centre_list[-1]
    for idx in range(len(centre_list)):
        gridline_list_left[idx+1] = ( 
            gridline_list_left[idx] +   
                2.*(centre_list[idx] - gridline_list_left[idx]))
        gridline_list_right[-idx-2] = (
            gridline_list_right[-idx-1] +   
                2.*(centre_list[-idx-1] - gridline_list_right[-idx-1]))
    ...
    weight = np.concatenate(([0.],np.linspace(0,1,len(centre_list)-1),[1.]))
    gridline_list = (1-weight)*gridline_list_left + weight*gridline_list_right
    return gridline_list


def concentration_profile(data_field,free_volume_field,limit_list=None):
    ...
    # This should be computed with highest value first, so input must be descending
    desc_cum_volume = np.cumsum(asc_volume[::-1])
    # Add a zero at the end for limits that are above max data value
    asc_cum_volume = np.concatenate((desc_cum_volume[::-1],np.zeros(1)))
